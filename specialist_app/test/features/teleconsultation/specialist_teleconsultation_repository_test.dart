...
void main() {
  group('SpecialistTeleconsultationRepository', () {
    late Dio dio;
    late DioAdapter adapter;
    late SpecialistTeleconsultationRepository repository;
    ...
    setUp(() {
      ...
      repository = SpecialistTeleconsultationRepository(client);
    });

    test('startSession returns TeleSession on 200', () async {
      adapter.onPost(
        '/teleconsultations/appt-1/session',
        (server) {
          server.reply(200, {
    ...
    test('buildIceServers merges session and TURN credentials', () {
      final session = standardSerializers.deserializeWith(
        TeleSession.serializer,
        {
          'appointment_id': 'appt-1',
          'room_id': 'room-1',
          'ice_servers': [
